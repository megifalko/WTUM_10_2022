...
train_photos = dataset.filter(lambda img, label: label == trainB)
train_vangoghs = dataset.filter(lambda img, label: label == trainA)
test_photos = dataset.filter(lambda img, label: label == testB)
test_vangoghs = dataset.filter(lambda img, label: label == testA)
...
train_vangoghs = train_vangoghs.cache().map(
    preprocess_image_train, num_parallel_calls=AUTOTUNE).shuffle(
    BUFFER_SIZE).batch(BATCH_SIZE)
...
test_vangoghs = test_vangoghs.map(
    preprocess_image_test, num_parallel_calls=AUTOTUNE).cache().shuffle(
    BUFFER_SIZE).batch(BATCH_SIZE)

sample_photo = next(iter(train_photos))
sample_vangogh = next(iter(train_vangoghs))
...
for epoch in range(EPOCHS):
  ...
  n = 0
  for image_x, image_y in tf.data.Dataset.zip((train_photos, train_vangoghs)):
